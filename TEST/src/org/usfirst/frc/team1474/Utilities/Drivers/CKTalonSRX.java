...
public class CKTalonSRX extends TalonSRX {					//Class that does error checking on set operations as robot functions go along
	...
	public void selectProfileSlot(int slotIdx, int pidIdx) {
		...
		if (currentSelectedSlot < mCLRampRate.length && currentSelectedSlot < mMMAccel.length && currentSelectedSlot < mMMVel.length) {
			...
			if (retryCounter >= Constants.kTalonRetryCount || !setSucceeded);
				//jConsoleReporter.report("Failed to change Talon ID" + getDeviceID() +  " profile slot!!!", MessageLevel.DEFCON1);
		}
	}
